...
![Integrate Stripe in Kadabra](/images/kadabra-stripe.webp)

Kadabra now lets you set up Stripe entirely through **chat**.
...
**Chat‑driven auto‑setup (recommended)**  
   ...
   Kadabra generates the checkout / portal edge functions, database tables with RLS, and UI buttons—no manual coding or webhooks unless you ask for them.


<Note>
  * For **one‑off sales**, be sure your cart or product page already works.
  * For **subscriptions**, confirm Supabase Auth is in place so Kadabra can link Stripe customers to each user’s **id**
</Note>
...
- **Webhooks are opt‑in.** Kadabra relies on edge‑function polling unless you request webhooks.  
...
- The project **must** be connected to Supabase. [Learn more about Supabase](https://docs.kadabra.dev/integrations/supabase)
...
Kadabra now generates all Stripe logic for you. Once your **Stripe Secret Key** is configured via the in-chat **Add API Key** form and your project is **connected to Supabase**, simply tell Kadabra what you need in chat—no manual Payment Links required.
...
<Steps>
  ...
  <Step title="Step 3">
    **Review & apply**
    Kadabra auto‑scaffolds the Edge Functions, database tables, and UI components (all tied to the user's id in Supabase). Check the preview, then click **Apply** to deploy.
  </Step>
</Steps>
...
For complex payment structures such as **subscriptions and role-based access**, Kadabra recommends using **Supabase** to securely handle Stripe integration. This allows for proper webhook handling, subscription management, and role-based access control based on payment tiers.
...
<Steps>
  <Step title="Connect Supabase to Your Project">
    Getting started is simple. Kadabra makes connecting Supabase effortless with a built-in native integration:

    1. Click the **Supabase button** in the top right corner of Kadabra.
  ...
  <Step title="Secure Payment Processing">
    Initiate the process by prompting Kadabra:

    <Note>
      Let's connect Stripe to my project. We will begin with secure payment processing.
    </Note>
    Kadabra will generate the necessary SQL schema for handling payments. This includes database tables for users, subscriptions, and payments. You can review and customize these tables to fit your specific product needs before applying changes.
  </Step>
    ...
    <Steps>
      ...
      <Step title="Step 2">
        **Go to Stripe Dashboard** \> Developers \> Webhooks \> Create an Event Destination.
        ![Stripe API Key](https://kadabra.dev/content/news/stripe-api.png)
      </Step>
      <Step title="Step 3">
      ...
      <Step title="Step 5">
        **Copy the Webhook Secret** and store it securely in **Supabase → Edge Functions → Manage Secrets → Add New Secret**.
        ...
        <Info>
          If unsure about naming the secret, ask Kadabra in **chat mode** for guidance.
        </Info>
      </Step>
    </Steps>
  </Step>
  <Step title="Securely Add API Keys">
    ...
    <Steps>
      ...
      <Step title="Step 2">
        **Copy the Secret Key** (DO NOT paste it directly in Kadabra chat).
      </Step>
      <Step title="Step 3">
        **Use the Kadabra "Add API Key" feature** to securely store it.

        <Warning>
          **Important Security Warning**

          Never paste your **Secret Key** directly in Kadabra chat. Treat it like the keys to your house—exposing it could allow unauthorized access to your Stripe account. Instead, store it securely using Kadabra's API key feature.
        </Warning>
      </Step>
    </Steps>
  </Step>
...
<AccordionGroup>
  <Accordion title="Check Console Logs">
    <Steps>
      ...
      <Step title="Step 3">
        Copy error messages and ask Kadabra for debugging assistance.
      </Step>
    </Steps>
  </Accordion>
  ...
  <Accordion title="Use Kadabra Chat Mode">
    <Steps>
      <Step title="Step 1">
        Switch to **chat mode** and ask Kadabra follow-up questions.
      </Step>
      <Step title="Step 2">
        Ask Kadabra follow-up questions.
      </Step>
      <Step title="Step 3">
