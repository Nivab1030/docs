...
Kadabra has native seamless integrations to export code and to add a backend. But it is also possible to integrate an external API if you are looking to add that specific functionality to your website.
...
You can read more about the power of APIs in [this article on our blog](https://kadabra.dev/blog/2025-01-28-the-power-of-apis).
...
<CardGroup cols="2">
  <Card title="GitHub Integration" icon="github" href="github">
    Understand how to integrate Kadabra with GitHub to export your code and for seamless version control.
  </Card>
...
Verified integrations work best with Kadabra:
...
<CardGroup cols="3">
  ...
  <Card title="Figma" href="https://kadabra.dev/blog/2025-01-22-figma-to-kadabra-builder-io-native-integration">
...
You can always request new integrations [here](https://feedback.kadabra.dev/?b=675883fb92175ba92d835f92).

## Using Any API with Kadabra
...
- **Internal systems with custom APIs**\
  ...
  You want Kadabra to pull from or send data to these systems to automate workflows or power user interfaces.
- **Third-party APIs Kadabra doesn’t yet support**\
  Maybe you use a niche SaaS product, a specialized data provider, or a partner integration that Kadabra doesn’t have built-in knowledge of.
  Even though it’s not officially supported, you still want Kadabra to connect, make calls, and work with the data.

In both cases, the challenge is the same: Kadabra doesn’t “know” the API by default. But with good documentation and authentication setup, it can still use it effectively.
Either way, the approach is the same: **Kadabra can still connect to it**. Here’s how.

<Steps>
  <Step title="Give Kadabra the context it needs">
    Kadabra works best when it understands the API. You can provide:

    - **OpenAPI specification** (if available)
    - **Copy-pasted docs or examples** (especially endpoints and sample requests/responses)

    The clearer the documentation, the better Kadabra can reason about how to call the API.
  </Step>
  <Step title="Handle authentication safely">
    ...
    **Option 1: Use Supabase Edge Functions as a proxy**
    ...
    - Let Kadabra call the API endpoint via the Edge Function (not the API directly)

    **Option 2: Implement the API integration directly in your Kadabra project**

    - Expose an authentication endpoint (e.g. to exchange credentials for a token or session)
    - Let Kadabra call that auth endpoint as part of the integration flow
