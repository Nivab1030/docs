...
Building with Kadabra makes coding faster and more intuitive—but even the best AI development tools can occasionally hit a snag. Sometimes your code won't run as expected, or you might notice unexpected behavior where the AI interprets your intent incorrectly.
...
<Steps>
  <Step title="Use the Try to Fix Button First">
    Click the **Try to Fix** button when an error shows up. Kadabra will scan your logs, detect the issue, and attempt a quick fix. It's your fastest first move. If that doesn't work, it's time to dig deeper.
  </Step>
  ...
  <Step title="Build in Logical Order">
    ...
    5. Prompt Kadabra to implement features.
  </Step>
  <Step title="Use Chat Mode for Clarity">
    When unsure, switch to **Chat Mode**:

    - Ask Kadabra to **analyze your project state**.
...
<AccordionGroup>
  ...
  <Accordion title="UI or Layout Problems">
    ...
    - Prompt Kadabra with: "Why is this element misaligned? Fix it."
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Error with Edge Functions">
    ...
    When using Kadabra with Supabase, always store sensitive API keys as Supabase Edge Function secrets rather than environment variables or in source code.
  </Accordion>
  ...
  <Accordion title="Connecting Supabase">
    To connect your Kadabra project to Supabase:
    ...
    4. Once connected, Kadabra can see your tables, RLS policies, and functions
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Branches are broken">
    ...
    - **Delete a branch:** Use the GitHub interface (not available directly in Kadabra)
    ...
    - Try refreshing the Kadabra editor
  </Accordion>
  <Accordion title="Repository sync issues">
    - Check your [GitHub permissions](https://docs.kadabra.dev/integrations/git-integration).
...
1. Use [Chat Mode](https://kadabra.dev/chat) for step-by-step help.
2. Submit a bug through our [Feedback Portal](https://feedback.kadabra.dev/).
3. If you're a paid customer, contact [Support](https://kadabra.dev/support).

## Comprehensive Debugging Manual

[This document](https://docs.google.com/document/d/1dfIy5rtkPN9Zi4smX1VAbfbhAiEyBvVw8p7B8kJiEVs/edit?tab=t.0) or [this website](https://kb-kadabra.netlify.app/?tab=errors#errors) were written by a Kadabra power user in our Discord community. 
